    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3">
        <div className="relative flex-1 w-full sm:max-w-xs">
          <MagnifyingGlassIcon className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-dark-400" />
          <input
            type="text"
            placeholder="Search by name or email..."
            value={searchQuery}
            onChange={(e) => setSearchQuery(e.target.value)}
            className="w-full pl-9 pr-3 py-2 bg-dark-800 border border-dark-600 rounded-lg text-sm text-foreground placeholder-dark-400 focus:outline-none focus:ring-2 focus:ring-primary-500"
          />
        </div>
        <select
          value={roleFilter}
          onChange={(e) => setRoleFilter(e.target.value as typeof roleFilter)}
          className="px-3 py-2 bg-dark-800 border border-dark-600 rounded-lg text-sm text-foreground focus:outline-none focus:ring-2 focus:ring-primary-500"
        >
        ...
        <button
          ...
          className="flex items-center gap-2 px-4 py-2 border border-primary-500 bg-transparent text-foreground rounded-lg hover:bg-primary-500/10 transition-colors text-sm ml-auto"
        >
      ...
      <div className="bg-dark-900 border border-dark-700 rounded-xl overflow-hidden">
        {isLoading ? (
          ...
          <div className="p-8 text-center text-dark-300">
            ...
            <p>{searchQuery || roleFilter !== 'all' ? 'No users match your filters' : 'No users found. Invite your first team member!'}</p>
          </div>
        ) : (
                ...
                <tr key={user.id} className="hover:bg-dark-800/30 transition-colors">
                  ...
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-2">
                      ...
                      {user.role_label && (
                        <span className="text-sm text-dark-400 hidden lg:inline">
                          {user.role_label}
                        </span>
                      )}
                    </div>
                  ...
                  {resetPassword ? (
                    <div className="space-y-4">
                      <div className="p-4 bg-success-500/10 border border-green-500/20 rounded-lg">
                        ...
                        <p className="text-dark-300 text-sm">New temporary password:</p>
                        <div className="mt-2 p-3 bg-dark-900 rounded-lg flex items-center justify-between">
                          <code className="text-foreground font-mono text-lg">
                            {resetPassword}
                          </code>
                    ...
                    <div className="space-y-4">
                      ...
                      <p className="text-dark-300 text-sm">
                        This will generate a new temporary password for this user. Their current
